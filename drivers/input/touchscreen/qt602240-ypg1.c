...
void TSP_forced_release(void)
{
	...
	if (fingerInfo[i].pressure > 0) {
		...
		input_report_abs(qt602240->input_dev, ABS_MT_PRESSURE, fingerInfo[i].pressure);
...
void  get_message(void)
{
	...
	} if ((quantum_msg[1] & 0x80) && (quantum_msg[1] & 0x40)) {
	/* Detect & Press */
...
} if ((quantum_msg[1] & 0x80) && (quantum_msg[1] & 0x10)) {
	/*Detect & Move */
		touch_message_flag = 1;
		fingerInfo[id].size = size;
#ifdef _SUPPORT_TOUCH_AMPLITUDE_
